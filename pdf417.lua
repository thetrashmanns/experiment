...
local pdf417 = {
	...
	quiet_v = 2,
	barcode_tbl = {},
	start_ptrn = "11111111010101000",
	stop_ptrn = "111111101000101001",
	txt_sub_modes = {
		alpha = { 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4a, 0x4b, 0x4c, 0x4d, 0x4e, 0x4f, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5a, 0x20, 0xFD, 0xFE, 0xFF },
		lower = { 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a, 0x6b, 0x6c, 0x6d, 0x6e, 0x6f, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7a, 0x20, 0xFD, 0xFE, 0xFF },
	...
	--Table of factors of the Reed-Solomon polynomial equations used for error correction; one sub array for each correction level (0-8).
	rsfactors = {
		ecl0 = { 0x01b, 0x395 },
		ecl1 = { 0x20a, 0x238, 0x2d3, 0x329 },
		ecl2 = { 0x0ed, 0x134, 0x1b4, 0x11c, 0x286, 0x28d, 0x1ac, 0x17b },
		ecl3 = { 0x112, 0x232, 0x0e8, 0x2f3, 0x257, 0x20c, 0x321, 0x084, 0x127, 0x074, 0x1ba, 0x1ac, 0x127, 0x02a, 0x0b0, 0x041 },
		ecl4 = {
...
function pdf417:create_code(code, ecl, aspect)
	...
	local rows = math.ceil(nce / columns)
	local size = columns * rows
	if rows < 3 or rows > 90 then
		if rows < 3 then
			rows = 3
		elseif rows > 90 then
			rows = 90
...
function pdf417:get_input_seq(code)
	...
	for i = 1, #num_seq do
				...
				if txtseq[2] > 1 then
					...
					if #prev_txt > 0 then
						...
						elseif #prev_txt % 6 == 0 then
							...
							seq_tbl[#seq_tbl + 1] = {901, prev_txt}
						end
					end
				end
		...
		if seq_len > 0 then
			seq_tbl[#seq_tbl + 1] = {902, seq[1]}
		end
		offset = seq[2] + seq_len
	end
	return seq_tbl
end
...
function pdf417:get_compact(mode, code, addmode)
	...
	if mode == 900 then
		...
		local txt_tbl_len = #txt_tbl
		if txt_tbl_len % 2 == 0 then
			txt_tbl[txt_tbl_len + 1] = 29
			txt_tbl_len = txt_tbl_len + 1
		end
		for i = 1, txt_tbl_len, 2 do
			cw[#cw + 1] = 30 * tonumber(txt_tbl[i]) + tonumber(txt_tbl[i + 1])
		end
	elseif mode == 924 then --Lua doesn't have switch statements ;-;
		local rest, sub_len;
		local code_len = #code
		while code_len > 0 do
			if code_len > 6 then
...
function pdf417.get_error_correction_lvl(ecl, numcw)
	local max_ecl = 8
	...
	if ecl < 0 or ecl > 8 then
		if numcw < 41 then
			ecl = 2
		elseif numcw < 161 then
			ecl = 3
		elseif numcw < 321 then
			ecl = 4
		elseif numcw < 864 then
			ecl = 5
		else
			ecl = max_ecl
		end
